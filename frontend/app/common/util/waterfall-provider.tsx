...
interface WaterfallProviderProps<T, S> {
  ...
  renderCachedComponents?: (context: {
    criteria: S;
    total: number;
    search: (c: S) => Promise<void>;
  }) => Record<string, ReactNode>;
}

// 创建 Context (初始化为 undefined，在 Hook 中做非空检查)
const WaterfallContext = createContext<
  WaterfallContextType<any, any> | undefined
>(undefined);
...
export function WaterfallProvider<T, S>({
  ...
  const mergeItems = useCallback(
    (currentItems: T[], newItems: (T | null)[]) => {
      // 1. 创建 Map，保持原有顺序
  ...
  const executeFetch = useCallback(
    async (targetPage: number, targetCriteria: S, isReset: boolean) => {
