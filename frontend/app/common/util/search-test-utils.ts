...
export type SearchFields<T extends Schema> = {
  -readonly [K in keyof T as T[K] extends { search: { fuzzy: any } }
    ? K
    : never]?: T[K] extends { search: { fuzzy: true } }
    ? string | { value: string; fuzzy?: boolean }
    : string;
};

// 仅提取 sort 相关的 key
export type SortFields<T extends Schema> = Partial<
  Record<
    Extract<
      { [K in keyof T]: T[K]["sort"] extends true ? K : never }[keyof T],
      string
    >,
    "asc" | "desc"
  >
