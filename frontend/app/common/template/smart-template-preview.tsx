...
export const SmartTemplatePreview: React.FC<SmartTemplatePreviewProps> = ({
  ...
  const { renderering, error, tooLarge } = useIntlayer(
  ...
  const render = useCallback(async () => {
    ...
    if (
      renderResultCache.current[`${angle}`] !== undefined &&
      renderResultCache.current[`${angle}`] !== null &&
      renderResultCache.current[`${angle}`] !== ""
    ) {
      ...
      <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-muted/20 p-6 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <span className="max-w-40 text-center text-[10px] leading-relaxed text-muted-foreground/60">
            {tooLarge.messageSecondaryPart1}
            <span className="italic text-nowrap p-0.5 px-1 rounded-md border bg-background/50">
              {template.path}
            </span>
            {tooLarge.messageSecondaryPart2}
          </span>
        </div>
      </div>
    );
