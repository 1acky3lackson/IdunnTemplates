...
const generateMeshGradient = (
  ...
  // 根据主题确定当前的 alpha 值
  const alpha =
    theme && theme.includes("dark") ? gradientAlphaDark : gradientAlphaLight;
  ...
  const gradients = pool.slice(0, 6).map((color, index) => {
    ...
    if (color.toLowerCase() === "#unknown") {
      ...
      if (pool[index - minValidIndex] !== "#unknown") {
        ...
        // 随机生成的颜色直接携带 alpha
        finalColorWithAlpha = generateRandomColor(
          seed++,
          Number.parseInt(alpha),
        );
      }
    } else {
...
export const TemplateCard: React.FC<TemplateCardProps> = ({
  ...
  template.colorSchemes = template.colorSchemes?.map((color) =>
  ...
  const handleAngleChange = useCallback(
    (newAngle: (0 | 1 | 2 | 3) | ((prev: 0 | 1 | 2 | 3) => 0 | 1 | 2 | 3)) => {
  ...
  const handleMove = async () => {
    try {
      const response = await IDUNN_API.apiV1TemplatesIdMovePatch(template.id, {
        path: newPath,
      });
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="secondary"
                size="icon"
                className="h-8 w-8 rounded-full bg-black/40 text-white hover:bg-black/60 backdrop-blur-md border-none"
              >
        ...
        <div className="flex items-center h-0 z-10 -mt-5">
          {template.colorSchemes && template.colorSchemes.length > 0 ? (
            <div className="flex items-center">
              {template.colorSchemes
                .slice(0, 6)
                .filter((c) => c !== null && !c.endsWith("unknown"))
                .map((color, idx) => {
                  ...
                  return (
                    ...
                    >
                      {isUnknown && (
                        <HelpCircle
                          className="w-1/2 h-1/2 text-muted-foreground/80"
                          strokeWidth={2.5}
                        />
                      )}
                    </div>
          ...
          <div className="mb-3 mt-4">
            ...
            <div className="flex flex-row align-middle justify-between gap-2">
              ...
              <div className="flex items-center gap-1 text-primary text-xs bg-primary/10 border px-1 py-0.5 rounded-md underline underline-offset-2">
                <User size={14} />{" "}
                {getUsernameByUuid(template.metadata.creatorId)}
              </div>
            </div>
            <p
              className="mt-1 line-clamp-1 text-[10px] font-mono text-muted-foreground/70 break-all"
              title={template.path}
            >
              {template.path}
            </p>
          ...
          <div className="flex items-center gap-1">
            <span className=" inline-flex flex-row align-middle justify-start gap-1">
              <div className="bg-primary/10 text-primary px-1.5 py-0.5 rounded text-[9px] font-bold inline-flex flex-row gap-1">
                <Milestone size={12} />
                <div>
                  {getDaytimeStringFromMsTimestampString(
                    template.latestVersionName,
                  ) || "1.0"}
                </div>
              </div>
      ...
      <Sheet open={isMoveSheetOpen} onOpenChange={setIsMoveSheetOpen}>
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>
              Change the path of the template. This will move the physical
              files.
            </SheetDescription>
          </SheetHeader>
