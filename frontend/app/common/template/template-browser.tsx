...
import {
  useWaterfall,
  useWaterfallCachedComponents,
  WaterfallProvider,
} from "../util/waterfall-provider";
import {
  MAX_SIZE,
  searchTemplatesObjectParam,
  templateSearchBatchBuildSortParams,
  templateSearchSortBuilder,
  type TemplateSearchParams,
} from "~/api/overrides/template-search-api";
...
function formSingleMetricsDesc(
  ...
  return (
    <span>
      {min} {min === 1 ? unitMono : unitPoly} ≤ {name} ≤ {max}{" "}
      {max === 1 ? unitMono : unitPoly}
    </span>
  );
}
...
export const TemplateFilters: React.FC<{
...
}> = ({ defaultFilterTab = "folders", disabledCatetories = [] }) => {
  // 获取翻译内容
  ...
  const [localMaxWidth, setLocalMaxWidth] = useState(
    criteria.maxWidth || MAX_SIZE,
  );
  const [localMaxHeight, setLocalMaxHeight] = useState(
    criteria.maxHeight || MAX_SIZE,
  );
  const [localMaxLength, setLocalMaxLength] = useState(
    criteria.maxLength || MAX_SIZE,
  );

  const [filterTabsValue, setFilterTabsValue] = useState<
    "folders" | "tags" | "creators" | "metrics"
  >(defaultFilterTab || "folders");

  const [creatorInfoList, setCreatorInfoList] = useState<
    Array<{ uuid?: string; name?: string }>
  >([]);

  const disabledSet = new Set(disabledCatetories);

  const { setUserInfo, getUsernameByUuid, getUuidByUsername } =
    useUserInfoCache();
  ...
  const isCreatorActive =
    criteria.creatorId !== undefined && criteria.creatorId !== "";
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-row items-center justify-between flex-wrap gap-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="lastVersionAt,desc">
                {filters.sortOrders.newlyUpdated}
              </SelectItem>
              <SelectItem value="lastVersionAt,asc">
                {filters.sortOrders.oldlyUpdated}
              </SelectItem>
              <SelectSeparator />
              <SelectItem value="metadata.creationTime,desc">
                {filters.sortOrders.newest}
              </SelectItem>
              <SelectItem value="metadata.creationTime,asc">
                {filters.sortOrders.oldest}
              </SelectItem>
              <SelectSeparator />
              <SelectItem value="metadata.lockedTimestamp,desc">
                {filters.sortOrders.newestLocked}
              </SelectItem>
              <SelectItem value="metadata.lockedTimestamp,asc">
                {filters.sortOrders.oldestLocked}
              </SelectItem>
              <SelectSeparator />
              <SelectItem value="metadata.height,desc">
                {filters.sortOrders.tallest}
              </SelectItem>
              <SelectItem value="metadata.width,desc">
                {filters.sortOrders.widest}
              </SelectItem>
              <SelectItem value="metadata.length,desc">
                {filters.sortOrders.longest}
              </SelectItem>
              <SelectSeparator />
              <SelectItem value="name,asc">
                {filters.sortOrders.nameAZ}
              </SelectItem>
              <SelectItem value="name,desc">
                {filters.sortOrders.nameZA}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="flex items-center justify-between space-x-2">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <Label htmlFor="locked-mode" className="font-bold text-foreground">
              {filters.lockedOnly}
            </Label>
          </div>
          <Switch
            ...
            onCheckedChange={(checked) =>
              handleFilterChange({ locked: checked ? true : undefined })
            }
          />
        </div>
        ...
        <Tabs
          defaultValue={defaultFilterTab}
          value={filterTabsValue}
          className="w-full"
          onValueChange={(val) => setFilterTabsValue(val as any)}
        >
          <TabsList className="w-full mb-2 gap-2">
            ...
            {!disabledSet.has("folders") && (
              <TabsTrigger
                value="folders"
                className="cursor-pointer hover:translate-px hover:shadow-md transition-all"
              >
                <HoverCard openDelay={10} closeDelay={100}>
                  ...
                  <HoverCardContent className="flex w-full flex-col gap-0.5 bg-background/50 backdrop-blur">
                    ...
                    <div className="text-sm">
                      {filters.tabs.folders.hoverDesc}
                    </div>
                    {isFolderActive && (
                      <div className="text-muted-foreground text-sm mt-1">
                        {filters.tabs.folders.currentValue}
                        <Badge>{criteria.pathPrefix}</Badge>
                      </div>
                    )}
                  </HoverCardContent>
                </HoverCard>
                ...
                <HoverCard openDelay={10} closeDelay={100}>
                  ...
                  <HoverCardContent className="flex w-full flex-col gap-0.5 bg-background/50 backdrop-blur">
                    <div className="font-semibold text-sm">
                      {filters.tabs.tags.hoverTitle}
                    </div>
                    <div className="text-sm">{filters.tabs.tags.hoverDesc}</div>
                    {isFolderActive && (
                      <div className="text-muted-foreground text-sm mt-1">
                        {filters.tabs.tags.currentValue}
                        <Badge>{criteria.pathPrefix}</Badge>
                      </div>
                    )}
                  </HoverCardContent>
              ...
              >
                <HoverCard openDelay={10} closeDelay={100}>
                  ...
                  <HoverCardContent className="flex w-full flex-col gap-0.5 bg-background/50 backdrop-blur">
                    <div className="font-semibold text-sm flex flex-row justify-between items-center">
                      ...
                      {isCreatorActive && (
                        <Button
                          size="xs"
                          variant="destructive"
                          onClick={clearCreatorConditions}
                          className="text-xs"
                        >
                    ...
                    <div className="text-sm">
                      {filters.tabs.creators.hoverDesc}
                    </div>
                    {isCreatorActive && (
                      <div className="text-muted-foreground text-sm mt-1">
                        {filters.tabs.creators.currentValue}
                        <Badge>
                          {
                            creatorInfoList
                              .filter((i) => i.uuid === criteria.creatorId)
                              .at(0)?.name
                          }
                        </Badge>
                      </div>
              ...
              >
                <HoverCard openDelay={10} closeDelay={100}>
                  ...
                  <HoverCardContent className="flex w-full flex-col gap-0.5 bg-background/50 backdrop-blur">
                    <div className="font-semibold text-sm flex flex-row justify-between items-center">
                      ...
                      {isMetricsActive && (
                        <Button
                          size="xs"
                          variant="destructive"
                          onClick={clearMetricsConditions}
                          className="text-xs"
                        >
                    ...
                    <div className="text-sm">
                      {filters.tabs.metrics.hoverDesc}
                    </div>
                    {isMetricsActive && (
                      <div className="text-muted-foreground text-sm mt-1">
                        <span className="mr-2">
                          {filters.tabs.metrics.currentValue}
                        </span>
                        {generateMetricsDescString(
                          criteria,
                ...
                <div className="space-y-2">
                  ...
                  <div className="w-full px-2 py-4 border rounded-md bg-background/50">
                    <CreatorPicker
                      ...
                      onValueChange={(val) =>
                        handleFilterChange({ creatorId: val })
                      }
                      // multiple={true}
          ...
          {!disabledSet.has("metrics") && (
            <TabsContent value="metrics">
              <div className="space-y-6 p-1">
                {[
                  {
                    label: filters.minLength,
                    val: localMinLength,
                    setLocal: setLocalMinLength,
                    key: "minLength",
                    type: "min",
                  },
                  {
                    label: filters.maxLength,
                    val: localMaxLength,
                    setLocal: setLocalMaxLength,
                    key: "maxLength",
                    type: "max",
                  },
                  {
                    label: filters.minHeight,
                    val: localMinHeight,
                    setLocal: setLocalMinHeight,
                    key: "minHeight",
                    type: "min",
                  },
                  {
                    label: filters.maxHeight,
                    val: localMaxHeight,
                    setLocal: setLocalMaxHeight,
                    key: "maxHeight",
                    type: "max",
                  },
                  {
                    label: filters.minWidth,
                    val: localMinWidth,
                    setLocal: setLocalMinWidth,
                    key: "minWidth",
                    type: "min",
                  },
                  {
                    label: filters.maxWidth,
                    val: localMaxWidth,
                    setLocal: setLocalMaxWidth,
                    key: "maxWidth",
                    type: "max",
                  },
                ].map((item) => (
                    ...
                    <div className="flex justify-between">
                      ...
                      (item.type === "max" && item.val < MAX_SIZE) ? (
                        <span className="text-xs text-muted-foreground">
                          {item.type === "min" ? "≥" : "≤"} {item.val}{" "}
                          {filters.blocksUnit}
                        </span>
                      ) : (
                        <span className="text-xs text-muted-foreground">
                          {filters.notSpecified}
                        </span>
                      )}
                    </div>
                    <Slider
                      ...
                      onValueCommit={(val) =>
                        handleFilterChange({ [item.key]: val[0] })
                      }
                    />
                  </div>
                ))}
...
function isDefaultCriteria(criteria: TemplateSearchParams) {
  return (
    criteria.sort === "metadata.creationTime,desc" && criteria.pathPrefix === ""
  );
}
...
const TemplateBrowserView: React.FC<{
  ...
  useEffect(() => {
    ...
    if (isDefaultCriteria(criteria)) {
      const saved = localStorage.getItem(
        `template-browser-criteria-${browserId}`,
      );
      if (saved) {
        try {
  ...
  const [localParams, setLocalParams] =
    useState<TemplateSearchParams>(criteria);
      ...
      {sidebar && (
        <aside className="hidden lg:block w-72 xl:w-96 border-r bg-muted/10 shrink-0 sticky top-0 h-screen">
          <div className="h-full flex flex-col">
            ...
            <ScrollArea className="flex-1 p-1">{TemplateFilters}</ScrollArea>
          </div>
        </aside>
      )}
          ...
          <div className="lg:hidden mr-2">
            <Sheet>
              ...
              <SheetContent side="left" className="w-[90vw] sm:w-95" forceMount>
                ...
                <div className="mt-4">{TemplateFilters}</div>
              </SheetContent>
            </Sheet>
          </div>
...
export const TemplateBrowser: React.FC<{
  ...
  const fetchTemplates = useCallback(
    async (page: number, criteria: TemplateSearchParams) => {
      ...
      <WaterfallProvider<Template, TemplateSearchParams>
        ...
        renderCachedComponents={() => {
          return {
            templateFilters: (
              <TemplateFilters
                defaultFilterTab={defaultFilterTab}
                disabledCatetories={disabledCategories}
              />
            ),
          };
        }}
