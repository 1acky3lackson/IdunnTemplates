...
export default function TopBar() {
  ...
  const [isCommercialAdmin, setIsCommercialAdmin] =
    React.useState<boolean>(false);

  React.useEffect(() => {
    IDUNN_API.apiV1CommercialAdminGet().then((data) => {
      if (data.status >= 200 && data.status < 300) setIsCommercialAdmin(true);
    });
  });
          ...
          {isAuthenticated ? (
            <HoverCard openDelay={10} closeDelay={100}>
              ...
              <HoverCardContent className="flex w-64 flex-col gap-2">
                <Button variant="default" onClick={() => nav("/me")}>
                  {meBtn}
                </Button>
                {isCommercialAdmin && (
                  <Button
                    variant="outline"
                    onClick={() => nav("/commercial/admin")}
                  >
                    商务管理员页
                  </Button>
                )}
