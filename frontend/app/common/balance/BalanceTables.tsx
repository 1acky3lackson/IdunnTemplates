    ...
    <GenericCrudTable<BalanceInfo>
      ...
      schema={{
        ...
        'availableBalance': { 
          title: '账户余额',
          sortable: true,
          render: (val) => <span className="text-emerald-600 font-semibold">{formatMoney(val)}</span>
        },
        'frozenBalance': { 
          title: '保留中',
          render: (val) => <span className="text-yellow-500">{formatMoney(val)}</span>
        },
        'pendingBalance': { 
          title: '待网易提现（预估）',
  ...
  return (
    <GenericCrudTable<UserBalanceRecordDto>
      ...
      schema={{
        ...
        'balanceBefore': { 
          title: '变动前余额',
          render: (val) => <span className="text-muted-foreground text-sm">{formatMoney(val)}</span>
        },
        'balanceAfter': { 
          title: '变动后余额',
          render: (val) => <span className="text-muted-foreground text-sm">{formatMoney(val)}</span>
        ...
        'relatedId': { 
          title: '关联业务ID',
          filterable: true,
          render: (val) => val ? <span className="font-mono text-xs bg-gray-100 p-1 rounded">{val}</span> : '-'
        },
        'description': { 
          title: '备注说明',
