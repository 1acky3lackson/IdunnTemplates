...
public class NeteaseOrderUpdateService {
        ...
        public boolean checkoutEnterToCalculated(NeteaseOrder order) {
                ...
                // 检查各角色至少有一人
                if (!contributors.containsKey(CommercialRoleType.BUILDER)
                                || contributors.get(CommercialRoleType.BUILDER).isEmpty() ||
                                !contributors.containsKey(CommercialRoleType.MODIFIER)
                ...
                for (int i = 0; i < builderContribs.size(); i++) {
                        ...
                        BigDecimal personalRatio = builderGroupRatio
                                        .multiply(BigDecimal.valueOf(c.getContributeRatio()))
                                        .setScale(8, RoundingMode.HALF_UP);
                ...
                for (int i = 0; i < uploaderContribs.size(); i++) {
                        ...
                        BigDecimal personalRatio = uploaderGroupRatio
                                        .multiply(BigDecimal.valueOf(c.getContributeRatio()))
                                        .setScale(8, RoundingMode.HALF_UP);
                        checkoutDetailService.createCheckoutDetail(
                                        c.getUsername(),
                ...
                for (int i = 0; i < modifierContribs.size(); i++) {
                        ...
                        BigDecimal personalRatio = modifierGroupRatio
                                        .multiply(BigDecimal.valueOf(c.getContributeRatio()))
                                        .setScale(8, RoundingMode.HALF_UP);
                        checkoutDetailService.createCheckoutDetail(
                                        c.getUsername(),
